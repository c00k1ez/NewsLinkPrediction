...
class NegariveSampler:
    ...
    def negative_samples(
        self, data: Dict[str, Union[torch.FloatTensor, torch.cuda.FloatTensor]]
        ...
        mask = torch.ones(anchor.shape[0], pos.shape[0]).type_as(pos) - torch.diag(torch.ones(pos.shape[0])).type_as(
            pos
        ...
        if self.margin is not None:
            ...
            dist_matrix = dist_matrix * mask
            dist_matrix[dist_matrix == 0.0] = float("-inf")
...
class OnlineTripletLoss(torch.nn.Module):
    def __init__(
        self, margin: int, sampler=NegariveSampler, reduction="mean", return_logits=False, number_of_neg_samples=1
    ):
...
class OnlineBCELoss(torch.nn.Module):
    def __init__(self, margin=None, sampler=NegariveSampler, reduction="mean", number_of_neg_samples=1):
    ...
    def forward(self, model_outputs: Dict[str, Union[torch.FloatTensor, torch.cuda.FloatTensor]]):
        ...
        # shape [batch_size, number_of_neg_samples, embedding_size]
        neg = self.sampler.negative_samples(model_outputs)
        ...
        # shape [batch_size*number_of_neg_samples,]
        anchor_neg_dist = self._distance(anchor_neg, neg)
